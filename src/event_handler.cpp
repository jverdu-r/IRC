...
void EventHandler::handleClientEvent(int client_fd) {
    ...
    std::cout << "bytes_received: " << bytes_received << std::endl; // Depuración
    if (bytes_received < 0) {
        ...
        return; // Importante: salir de la función en caso de error
    }
    ...
    if (bytes_received == 0) {
        ...
        std::cout << "received_data: " << received_data << std::endl; // Depuración

        if (partial_messages.find(client_fd) != partial_messages.end()) {
            std::cout << "partial_messages[client_fd]: " << partial_messages[client_fd] << std::endl; // Depuración
        ...
        if (newline_pos == std::string::npos) {
            newline_pos = received_data.find("\n");
        }
        std::cout << "newline_pos: " << newline_pos << std::endl; // Depuración

        if (newline_pos != std::string::npos) {
            std::cout << "Datos recibidos del cliente " << client_fd << ": " << received_data;

            // Eliminar \r\n o \n
            received_data = received_data.substr(0, newline_pos);

            // Enviar todos los comandos a command_handler
            if (received_data.find('/') == 0) {
